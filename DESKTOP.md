...
- [ ] Scaffold the Wails app in `desktop/`, depending on the `cli` module
- [ ] Add `go.work` (use ./cli and ./desktop)
- [ ] Bind a minimal "receive by code" Go method that uses the engine
- [ ] Receive one file to disk from a peer
- [ ] Prove interop: receive a file sent from floe.one (browser) and from `floe` (CLI)
