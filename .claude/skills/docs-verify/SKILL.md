---
name: 'docs-verify'
description: 'Docs pre-flight for Floe. Runs docs-check.mjs (links, nav, frontmatter, link labels, env-var fan-out, em dashes), then walks the claim map to diff each user-facing claim against its source symbol. Use when editing docs/**, README.md, CONTRIBUTING.md, SELF_HOSTING.md, DESKTOP.md, or .github templates, or when changing a user-facing flag, limit, env var, default, or CLI string that the docs quote.'
---
...
From the repo root:
    ...
    node .claude/skills/docs-verify/scripts/docs-check.mjs
...
- env: presence gaps are NOTEs; the deliberate subsets in SELF_HOSTING.md and unraid/ are
  one info line each (the `CURATED` constant). Default contradictions are HARD.
...
- configuration.mdx is canonical for env vars. CONTRIBUTING.md (two tables),
  SELF_HOSTING.md, server/.env.example, .env.docker.example, docker-compose.yml and
  unraid/\*.xml duplicate parts of it. A key or default change touches all of them.
...
against its symbol, and the test plan names the preview pages you opened. Green is a
...
- CONTRIBUTING.md "Documentation" and "Pull Request Process" step 10
