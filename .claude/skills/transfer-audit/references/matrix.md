...
browser has no code UI, `client/components/P2PTransfer.tsx` `getRoomFromUrl` and `ROOM_ID_REGEX`). The
...
| ---------------- | ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| C sender         | link          | stdout box rows `Code` and `Link`; regex `/https?:\/\/\S*#room=[^\s]+/`                                                                                                                                                                   | `cli/cmd/floe/main.go` `runSend` (the `PrintBox` rows); `client/e2e/helpers.ts` `spawnSend` |
| C sender         | wait, connect | `  Waiting for peer...`, `  Connecting...`, `  Connected` (a CLI with `--relay-only` prints `  Connected (direct)` or `  Connected (relay)`)                                                                                              | `cli/cmd/floe/main.go` `runSend`                                       |
...
| C receiver       | connect       | `  Connecting to sender...`, `  Connected`, optional `  Peer version: <ver>`                                                                                                                                                              | `cli/cmd/floe/main.go` `runReceive`; `cli/engine/transfer/receiver.go` `ReceiveFilesWithOptions`                        |
...
| W receiver       | joined        | `Secure room joined`, `Waiting for the sender to start`                                                                                                                                                                                   | `client/components/P2PTransfer.tsx` (the receiver handshake pipeline)                                 |
...
| W receiver       | done          | `N file received` or `N files received`; `Download All` and `Download ZIP` when more than one file                                                                                                                                        | `client/components/P2PTransfer.tsx` (the receiver block)                       |
...
| D sender         | live          | status `Waiting for the receiver...`, `Peer connected. Sending...`; pill `Active` then `Direct` or `Relay`                                                                                                                                | `desktop/frontend/src/App.tsx` (the `send:status` handler); `desktop/app.go` `runSend`                        |
