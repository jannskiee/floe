...
<AccordionGroup>
  ...
  <Accordion title="How long does the sender have to stay connected?">
    ...
    If the sender closes the tab or window, quits, or cancels mid-transfer, the session ends. Nothing half-written is left behind: a CLI or desktop receiver deletes the file it was writing, and a browser receiver offers no download for the file that was in flight. Files that finished earlier in the same batch are kept. Either way, a file that did not arrive whole is never handed to you looking complete. That holds even when the receiving app is killed outright or the machine loses power: an unfinished file only ever exists as a `.part` file, never under its final name.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="I'm on iOS. Why are downloads behaving oddly?">
    ...
    The ZIP option only appears when a transfer contains more than one file. It shows up as soon as the second file lands, hides again while the next file streams, and settles once the transfer finishes. A single file has just its own download button, and that is the only way to save it.
  </Accordion>
