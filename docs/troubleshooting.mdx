...
<AccordionGroup>
  ...
  <Accordion title="Downloads fail or behave oddly on iOS">
    ...
    The **Download All** and **Download ZIP** buttons appear only for multi-file transfers. They show up as soon as the second file lands and hide again while the next file is streaming, so if they vanish mid-transfer nothing is wrong; they return between files and stay once the transfer finishes. A single file is saved from the download button on its own row in the list.
...
<AccordionGroup>
  ...
  <Accordion title="My short code does not work">
    ...
    That same message covers a typo, and codes are matched exactly. They are always lowercase words joined by hyphens, so `Olive-Tiger-Castle` is not the same code as `olive-tiger-castle`. Surrounding spaces are trimmed for you; nothing else is.
