...
every per-IP limiter: IPv6 addresses are keyed on their `/64` prefix, so everyone sharing a `/64`
shares one bucket, and IPv4-mapped IPv6 (`::ffff:1.2.3.4`) unwraps to plain IPv4 so the same
