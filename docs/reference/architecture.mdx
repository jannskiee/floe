...
| `ping` | callback function | Round-trip latency probe, not a keepalive. The browser sends one every 2 seconds and times the reply to drive the ping readout beside the connection dot. The server invokes the callback immediately. |
...
| `ping` | - | Server responds with `{ "type": "pong" }`. Supported, but no Floe client sends it: the CLI and the desktop app rely on WebSocket protocol-level pings instead (see below). |
...
`offset` is reserved for mid-file resume. Both senders honor a non-zero value and seek to it, but no receiver in any current client emits one: there is no resume state on disk or across connections, so in practice it is always 0. The ack carries the receiver's own `pv` and `pvMin` so the sender can verify compatibility from its side, plus `ver` when the receiver is a Go peer.

**Binary chunks (raw bytes)**

Both the Go engine (CLI and desktop) and the browser size chunks adaptively to the connection's negotiated maximum message size, capped at 256 KB. When that size is unavailable they fall back to different defaults: 16 KB in the Go engine, 64 KB in the browser. Chunk size is a local sender choice, not part of the wire protocol; receivers handle any size.
...
This lets a Go sender exit cleanly instead of polling the SCTP buffer. Browser receivers never send it, and the browser sender ignores it even when a Go receiver does: it always waits for the data channel buffer to drain to zero instead.
...
- When the ranges do not overlap, the receiver sends an `incompatible` message and each side prints an actionable message with a remedy for its own surface: the CLI says run `floe update`, the desktop app points at the Microsoft Store or floe.one/download, and the browser says refresh the page. The newer peer is told to ask the other side to update.
- Which side you should trust depends on who is sending. Go peers do not read the wire `reason` at face value: they rebuild the message locally from the frame's `pv`/`pvMin`, so the side they name is always correct. A Go receiver also words the `reason` it puts on the wire from the *recipient's* point of view for the benefit of peers that display it verbatim. A browser receiver does not do that second step. It sends the same sentence it shows itself, so a browser receiver can tell a CLI sender to "refresh the page". Go senders are unaffected because they rebuild; when the message looks wrong for your surface, compare the two version strings it prints and update the older one.
- `ver` is the human release string, and its exact shape depends on which build sent it. The CLI sends its version with no leading `v` (`1.10.4`), because GoReleaser strips it at build time. The desktop app sends the tag verbatim (`desktop-v0.2.7`). Anything built from source sends the literal `dev`. It is optional: Go peers always send it, browser peers never do. It is informational only, used for the peer-version note the CLI prints when the two releases differ, and it never gates a transfer.
