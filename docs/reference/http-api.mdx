...
Codes are three lowercase words joined by hyphens, drawn from the EFF short word list (1295 words;
its one hyphenated entry, yo-yo, is dropped) with a cryptographically secure random source. On a
collision with a live code the server draws again, up to ten times, and only then falls back to a
four-word phrase.
