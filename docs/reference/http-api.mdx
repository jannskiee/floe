...
Codes are three lowercase words joined by hyphens, drawn from the 1295-word EFF short word list
with a cryptographically secure random source. On a collision with a live code the server draws
again, up to ten times, and only then falls back to a four-word phrase.
