...
description: "Every HTTP endpoint the Floe signaling server exposes: health checks, room codes, TURN credentials, and the byte counter, with request and response shapes."
...
<ResponseField name="code" type="string" required>
  The phrase to share. It resolves for at most 10 minutes from this moment, and stops resolving
  earlier once the room pairs or empties.
</ResponseField>
...
A second `POST` for the same room retires that room's previous code, so a room only ever has one
working phrase and a retrying sender does not widen its own exposure.

The `503` is a global guard, not a per-IP one. It bounds the memory the code registry can use. A
slot is freed as soon as its code is retired, which happens when the room pairs, when the room
empties, when the same room registers again, or at expiry, whichever comes first. A sweeper clears
whatever is left over once a minute, so the count can briefly include codes that have already
expired.
...
| Status | Body | When |
|---|---|---|
| `404` | `{ "error": "Code not found or expired" }` | Unknown code, or one already retired: past its 10 minutes, or its room has paired or emptied |
| `429` | `{ "error": "Too many requests" }` | Over the per-IP code limit |
| `429` | `{ "error": "Too many requests" }` | Over the per-IP failed-lookup budget |

**Rate limit:** 60 requests per IP per 60 seconds, shared with `POST /api/code` and configurable
with `MAX_CODE_REQUESTS_PER_IP`.

**Failed-lookup budget:** 10 misses per IP per 60 seconds, configurable with
`MAX_FAILED_CODE_RESOLVES`. Only a `404` counts against it; a successful resolve costs nothing, so
retrying a code that works is always safe. The budget is checked before the lookup, so the two
`429`s above are indistinguishable from each other and from a refusal about a code that does not
exist. Neither ever carries a room id.

A resolve does not retire the code. A receiver that resolves and then fails before it can join,
with no relay credentials or an output path it cannot write, can retry the same phrase.
...
to in the client's environment (see [Configuration](/self-hosting/configuration)), and `null`
...
comma-separated list matches nothing. Requests with no `Origin` header always pass.

An HTTP request from a rejected origin does not come back as a recognizable CORS error. The server
answers `500` with `{ "error": "Internal server error" }` while the browser reports a generic CORS
failure. If you see that pairing, check `CLIENT_URL` first.

**WebSocket origins.** Both WebSocket paths check `Origin` against the same list, and also let in
an `Origin` naming the same host as the request's `Host` header (the port is compared only when
`Host` still carries one, since some proxies drop it). That second rule is how the CLI and the
desktop app reach a self-hosted server regardless of `CLIENT_URL`: they send the server's own
address as their origin. A handshake from any other origin is refused before the connection opens:
`/ws` answers `403`, and Socket.IO answers `400` with the body `Origin not allowed` on its WebSocket
transport and `403` on polling. The server logs the first refusal on each path, once, with the
`Host` it compared against.
