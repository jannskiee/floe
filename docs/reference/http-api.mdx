...
an `Origin` whose host matches the request's own `Host` header. That second rule is how the CLI and
the desktop app reach a self-hosted server regardless of `CLIENT_URL`: they send the server's own
...
transport and `403` on polling. The server logs the first refusal on each path, once.
