...
- **The end of a batch is a race, not a message.** A Go sender finishes on whichever comes
  first: the `received` frame, its send buffer reaching zero, or a full 60 seconds in which the
  buffer did not shrink at all.
...
<ResponseField name="ver" type="string">
  ...
  `desktop-v0.2.12`. A build from source sends `dev`. Browser peers omit the field entirely. It is
...
```json ack
{ "type": "ack", "id": "3f2b9c1e-...", "offset": 0, "pv": 1, "pvMin": 1, "ver": "1.10.11" }
```
...
| Sender waiting for delivery confirmation | 60 s with no drain | `timed out waiting for delivery confirmation from peer` |
...
| Receiver grace before teardown | 5 s | Lets the final acknowledgment reach the sender |
