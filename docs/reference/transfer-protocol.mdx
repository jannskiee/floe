...
cap from a sender, and a discarded file or a refused file description from a receiver.
...
Go receivers from v1.5.0 to v1.5.5 will write a binary one into the file being received.

The whole encoded frame stays under 1000 bytes, reason included, or a browser receiver stops
reading it as a control message. A sender caps the reason again for display, at 300 characters.

Whichever side sends it waits for the send buffer to drain, up to 2 seconds, before closing.
Without that wait the frame is routinely lost: closing a pion connection aborts the SCTP
association and discards whatever was still queued, so the peer would see the close and report a
lost connection rather than the reason.
