...
<ResponseField name="ver" type="string">
  The human release string, and its shape depends on the build. The CLI sends `1.10.7` with no
  leading `v`, because GoReleaser strips it. The desktop app sends the tag verbatim, as
  `desktop-v0.2.9`. A build from source sends `dev`. Browser peers omit the field entirely. It is
...
```json ack
{ "type": "ack", "id": "3f2b9c1e-...", "offset": 0, "pv": 1, "pvMin": 1, "ver": "1.10.7" }
```
