...
These apply to the floe.one website only. The CLI and the desktop app contain no analytics, no error monitoring, and no telemetry. Beyond the transfer itself, they make exactly two kinds of optional request, and both have an off switch: the byte-count report described above, and an update check.

**The update check:** once a day, the desktop app (GitHub builds 0.2.3 and later) asks GitHub whether a newer desktop release exists, and the CLI does the same only when you run `floe version`. The request carries nothing about you beyond what any web request reveals (your IP address and a user agent); nothing about your files or transfers is in it, and nothing downloads or installs by itself. Turn off **Check for updates** in the desktop [Settings](/desktop/settings#check-for-updates), or set `FLOE_NO_UPDATE_CHECK=1` for either surface, and no request is made at all. Microsoft Store builds never check; the Store updates them itself.
...
- The DTLS handshake verifies each peer's certificate against the fingerprint exchanged during signaling, so an outside attacker on the network cannot inject a different certificate. Like every WebRTC application, Floe trusts the signaling server to relay those fingerprints faithfully. To check that they were relayed faithfully, both devices show the same eight-digit verification code once a transfer connects, derived from the two certificate fingerprints (the safety number model used by messaging apps). Compare the codes over any channel you trust; if they differ, the connection was intercepted, so stop the transfer. Checking is optional, and both devices need a Floe version that displays the code. Running your own instance removes the trust dependency entirely (see [Self-hosted instances](#self-hosted-instances)).
