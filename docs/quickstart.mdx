...
3. Each file gets its own download button the moment it arrives, so you can save one while the rest are still coming.
4. On a transfer of two or more files, two more buttons sit below the list: **Download All** saves every file separately, and **Download ZIP** bundles them into one archive. Both are hidden while a file is streaming and reappear between files, so wait until the transfer finishes if you want everything at once. A single-file transfer shows only that file's own button.
