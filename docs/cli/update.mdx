...
<Note>
  ...
  Microsoft Store build updates itself, while the GitHub `.exe` and `.zip` builds (0.2.3 and
  later) show a daily update notice but never install anything, so you download and run the
  current build again. `floe update` never touches the desktop app, and desktop builds ship on
  their own `desktop-v*` tags as GitHub pre-releases, so they are never picked up by
  `floe update` or the install scripts. See
