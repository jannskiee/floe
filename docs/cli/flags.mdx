...
| `--version` | `-v` | `false` | Print the installed version and exit, for example `floe 1.10.11`. Root only: `floe --version` works, `floe send --version` does not. See [Alternatives](/cli/version#alternatives). |
...
| `130` | Interrupted, by `Ctrl+C` or by `SIGTERM`. Floe prints `Canceled.` to stderr and removes the `.part` file it was writing. |
...
A `floe receive` can sit for up to about ten seconds after printing its summary. That is a five-second
grace period so the final acknowledgment reaches the sender, plus the report. The full
