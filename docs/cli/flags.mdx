...
| `--version` | `-v` | `false` | Print the installed version and exit, for example `floe 1.10.9`. Root only: `floe --version` works, `floe send --version` does not. See [Alternatives](/cli/version#alternatives). |
