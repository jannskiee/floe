...
<Update label="Unreleased" tags={["CLI", "Web", "Desktop"]}>
  **Every transfer now shows a verification code, and failed receives clean up their partial file**

  - `floe send` and `floe receive` print a Verify line once the connection is up: the same eight digits on both devices, derived from the connection's own certificate fingerprints. Compare them over any channel you trust to rule out anyone sitting between you, even the signaling server; on a mismatch, stop the transfer. Checking is optional and nothing waits on it. A peer on an older version shows nothing to compare.
  - Web app: the transfer card shows the same code once connected, on both the sending and the receiving side, and keeps it visible after the transfer finishes so a fast transfer can still be compared.
  - Desktop: a quiet Verify row under the progress bar shows the same code, with the explanation in its tooltip. It stays visible after the transfer finishes, until the next transfer or Start over.
