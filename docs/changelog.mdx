...
<Update label="Unreleased" tags={["CLI", "Web", "Desktop"]}>
  **A file name or version string chosen by the other side can no longer rewrite the terminal, disguise its extension in the accept prompt, or crash the receiver**
  ...
  - A sender that announced a negative or impossibly large file size made the receiver crash before the prompt appeared, and in Floe Desktop that ended the whole app. Sizes and file counts are now checked as the announcement arrives, an announcement that cannot be right ends the transfer with an error instead, and `floe send` shows that reason rather than only reporting a closed connection.
  ...
  - A receiver that asks to resume from a position outside the file is refused by `floe send` and by the web app, instead of the transfer hanging or sending nothing.
  ...
  - Web app: the error shown when a file arrives incomplete, and the message shown when the other side rejects a transfer, have control characters and text-direction marks removed and are shortened before they appear on the page.
  - Desktop: the incoming preview shows the cleaned name, the progress line and History show the name as it was saved, the status line shows the cleaned messages from the shared engine, and the crash above no longer takes the app down.
