...
A freed seat does not let just anyone in. Once devices on two different networks have joined a
room, a device on a third network is turned away as if the room were full, for as long as the room
exists. Devices that share one network address count as one, so two people behind the same router
are not told apart.
