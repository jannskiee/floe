...
A **browser receiver** keeps each incoming file in memory until that file is complete, then hands
it to the browser as a download. A large enough file exhausts the tab, and on a phone that happens
sooner. Floe Desktop and the CLI write straight to disk and have no such ceiling, which makes them
the better choice for anything multi-gigabyte. **Download ZIP** compounds it, because zipping
holds every file at once.
