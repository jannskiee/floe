...
<Note>
  The variable is `FLOE_IMAGE_TAG`, not `FLOE_VERSION`. `FLOE_VERSION` belongs to the [CLI installer](/cli/installation) and is written with a leading `v`, as in `v1.9.1`. Image tags carry no `v`, so reusing that name would resolve to a tag that does not exist.
</Note>
