...
| `1.10.7` | One exact release | You want to control when you move between versions. |
...
```dotenv .env
FLOE_IMAGE_TAG=1.10.7
```

The variable is `FLOE_IMAGE_TAG`, not `FLOE_VERSION`. `FLOE_VERSION` belongs to the [CLI installer](/cli/installation) and is written with a leading `v`, as in `v1.10.7`. Image tags carry no `v`, so reusing that name would resolve to a tag that does not exist.
