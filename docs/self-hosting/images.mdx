...
| `1.10.10` | One exact release | You want to control when you move between versions. |
| `1.10` | The newest patch of a minor line | You want patches automatically but not minor bumps. |
| `main` | The newest build of `client/` or `server/` on `main` | You want changes before they are released. `main` only advances by merged pull request, and every image is smoke-tested on real hardware before the tag moves. A docs-only merge leaves this tag where it is, since it starts no build. |
...
```dotenv .env
FLOE_IMAGE_TAG=1.10.10
```

The variable is `FLOE_IMAGE_TAG`, not `FLOE_VERSION`. `FLOE_VERSION` belongs to the [CLI installer](/cli/installation) and is written with a leading `v`, as in `v1.10.10`. Image tags carry no `v`, so reusing that name would resolve to a tag that does not exist.
...
The digests a tag is built from are validated before it moves, and every tag is checked again afterwards, so a tag and its digest always refer to an image that was actually started and exercised. The second pass is detection rather than prevention: it runs after the tag moved, and its job is to make a bad publish loud instead of silent.
