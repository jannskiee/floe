...
Credentials are minted from Cloudflare and then **cached in memory for 5 minutes**. Two consequences worth planning around: a rotated Cloudflare key takes effect within about 5 minutes without a restart, and if the Cloudflare API starts failing the server keeps serving the last good copy for up to about 23 hours after it was minted (logging `Cloudflare TURN mint failed` at most once every 5 minutes) before it falls back, so a broken key can look healthy for a while.
