...
<Steps>
  <Step title="Create the coturn config">
    ...
    This file is gitignored because it holds your shared secret, so it does not exist in a fresh clone. Skip this step and Docker creates a directory at that path, and coturn fails to start.
  ...
  <Step title="Start the stack with the TURN profile">
    ...
    <Warning>
      Create `coturn/turnserver.conf` first, as the previous step describes. If that path does not exist, Docker creates a **directory** there and coturn fails to start with a confusing error. Downloading it alongside the compose file works too:

      ```bash
      curl -fsSL --create-dirs \
        https://raw.githubusercontent.com/jannskiee/floe/main/coturn/turnserver.conf.example \
        -o coturn/turnserver.conf
      ```
    </Warning>
  </Step>
</Steps>
