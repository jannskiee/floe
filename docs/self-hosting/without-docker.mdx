...
The signaling server needs **Node.js 18 or later**, for the built-in `fetch` and
`crypto.randomUUID` it relies on. `package.json` declares no `engines` field, so nothing stops you
starting it on something older; it will fail at runtime instead. CI runs it on Node 20 and the
published image is built on Node 24, so anything in that range is well trodden.
