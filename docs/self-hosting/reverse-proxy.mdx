...
Both apps send the address you give them as their `Origin`. Here that address is
your `CLIENT_URL`, which the server always accepts, so nothing about the `Host`
header affects them on this layout. `Host` matters only when the apps use an
address that is not exactly your `CLIENT_URL`, as on the
[two-hostname layout](/self-hosting/production).
