...
send your server's own address as their origin, which the server accepts as long as your
proxy keeps the `Host` header, so they reach your server regardless of that setting.
