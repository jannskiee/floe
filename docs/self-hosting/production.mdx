...
<Steps>
  <Step title="Put both services behind a reverse proxy with HTTPS">
    ...
    Keep the host name in the `Host` header as the client sent it. Caddy and Traefik do by default, and nginx does with `proxy_set_header Host $host`. The CLI and the desktop app connect to `api.your-domain.com`, which is not your `CLIENT_URL`, so the server lets them in by matching their `Origin` against the host name in `Host`. A proxy that rewrites `Host` to the upstream address (`localhost:3001`, the nginx default without that line) gets them refused with a `403`.
  </Step>
  ...
  <Step title="Point the CLI and the desktop app at your instance">
    ...
    In the desktop app, open Settings, expand **Advanced**, and fill in both **Server address** (`https://api.your-domain.com`) and **Share link address** (`https://app.your-domain.com`), then click **Test**. See [Desktop app settings](/desktop/settings#server).
  </Step>
</Steps>
