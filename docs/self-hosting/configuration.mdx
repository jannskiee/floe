...
| `FLOE_IMAGE_TAG` | compose | No | `main` | Which published image tag to run, for example `1.9.0` or `sha-7a1ec76`. See [Container Images](/self-hosting/images). Not to be confused with `FLOE_VERSION`, which belongs to the CLI installer and carries a leading `v`. |
