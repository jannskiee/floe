...
| `FLOE_IMAGE_TAG` | compose | No | `latest` | Which published image tag to run, for example `1.10.9`, `1.10`, `main`, or `sha-fed7ebb`. See [Container images](/self-hosting/images). Not to be confused with `FLOE_VERSION`, which belongs to the CLI installer and carries a leading `v`. |
