...
| `FLOE_SOCKET_URL` | client (runtime) | No | _(derived)_ | Complete signaling server origin for a prebuilt client image. Takes precedence over the build-time URL. If empty, the client uses the browser hostname and `FLOE_SOCKET_PORT`. |
| `FLOE_SOCKET_PORT` | client (runtime) | No | `3001` | Signaling server host port used when `FLOE_SOCKET_URL` is empty. |
