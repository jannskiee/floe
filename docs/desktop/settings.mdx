...
A pass also reports whether the server offers a **relay**. Only
[Hide my IP](#hide-my-ip-address) needs one, so a server without a relay still passes; the
message says so rather than reading as a failure.

| Result | Meaning |
|---|---|
| Connected. | All three stages passed, and the server offers a TURN relay. |
| Connected. This server has no TURN relay, so Hide my IP will not work. | All three stages passed. The server is fine for ordinary transfers, and only [Hide my IP](#hide-my-ip-address) cannot work on it. See [TURN relay](/self-hosting/turn-relay). |
