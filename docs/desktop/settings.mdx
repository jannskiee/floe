...
While the check runs, the line reads `Checking the server...`. A pass reports `Connected.` on its
own; every failure carries an `Error: ` prefix, so the second row below reaches the screen as
...
Settings live in a plain JSON file at `%APPDATA%\floe\desktop.json`. Floe writes it the first time
a setting changes, so on a new install the file does not exist yet and the app runs on its
defaults. Once written, it looks like this:
