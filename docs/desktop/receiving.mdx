...
bar shows the current file with speed and time remaining. **Cancel** stops it. A small
**Verify** row under the progress bar shows the connection verification code; the sender shows
the same eight digits, and comparing them rules out an intercepted connection. It is optional.
