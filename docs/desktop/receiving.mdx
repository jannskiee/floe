...
**Save to** is where everything arrives. Leave it blank and Floe uses your **Downloads** folder, or
...
sticks are, or on a network share that does not support it, the mark cannot be written and the
file arrives untagged. The file itself is saved either way.
...
anywhere. Remove one entry with **Remove** inside its open row, or **Clear** in the History header
to remove all of them. Clear asks first, replacing itself with **Clear all?** and a **Yes** and a
**No**, so this one cannot go wrong in a single click. Clearing history does not touch the files themselves, and it is
