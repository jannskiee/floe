...
<Warning>
  ...
  you typed has not been sent yet. Starting over will clear it."

  Three things reset with no prompt at all: staged files on their own, a send or a receive that
  is still connecting, and anything typed once a send has completed. That last one holds until
  you stage or remove a file, send again, cancel, or start over, so a note typed underneath a
  finished send can be cleared without being asked.
</Warning>
