...
<Warning>
  ...
  when there is an unsent text note (whether you typed it and clicked away, or reached Start
  over via the Floe lockup in the titlebar). The dialog reads "The text you typed has not been
  sent yet. Starting over will clear it." Staged files, in-flight sends that are still
  connecting, and the state right after a completed transfer deliberately reset without a
  prompt.
</Warning>
