...
<Note>
  ...
  use the [web app](/web-app/sending) or the [CLI](/cli/installation).
</Note>
...
<Tabs>
  <Tab title="Microsoft Store">
    ...
    - Updates are handled by the Store, so there is nothing to download by hand.
...
| GitHub release | Manual. Download the current installer and run it over your existing install, or unzip a new portable build. There is no in-app updater yet. |
...
right-click entry if you turned it on. It deliberately leaves two things behind, so a reinstall
